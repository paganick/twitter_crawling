...
def main():
    ...
    graph_filename = folder+'graph.csv'
    ...
    for source in range(number_of_nodes):
       ...
       #             print(target, 'target nodes checked.')
        #else:
            try:
                for page in tweepy.Cursor(api.friends_ids, screen_name=name_list[source]).pages():
                    friends = []
                    friends.extend(page)
                    for friend_id in friends:
                        if friend_id in id_list:
                            edge_list.append([id_list[source], friend_id])
                            edge = pd.DataFrame([[id_list[source], friend_id]], columns=['Source', 'Target'])
                            edges_data = pd.concat([edges_data, edge], ignore_index=True)
                            indegree[source]+=1
            except tweepy.TweepError:
                print ('Error checking friends of node', name_list[source])
    ...
    #indegree_data = pd.DataFrame(id_list, name_list, indegree, columns=['id','Label','indegree'])
    #indegree_data.to_csv(indegree_filename, mode='w', index=False)
